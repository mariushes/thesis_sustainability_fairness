...
from .presets import DB_LOSS_MAP
...
class GeneralDistiller(BasicDistiller):
    ...
    def __init__(self, train_config,
        ...
        if self.d_config.debias_loss_type:
            self.debias_loss = DB_LOSS_MAP[self.d_config.debias_loss_type]
            #print(f"### Debias loss mapped: {str(self.debias_loss)}")
        else:
            self.debias_loss = None
    ...
    def compute_loss(self,results_S,results_T):
        ...
        if self.debias_loss:
            debias_loss_value = self.debias_loss(results_S, results_T, self.model_S, self.t_config.device)
            print(f"Add Debias loss: {str(debias_loss_value)}")
            total_loss += debias_loss_value * self.d_config.debias_loss_weight
